...
const ChatPage = () => {
  ...
  const { messages, loading: messagesLoading, connectionStatus } = useSelector(
  ...
  // Улучшенная фильтрация сообщений с приведением типов
  const currentMessages = messages.filter((m) => {
    // Приводим оба значения к числу
    const msgChannelId = Number(m.channelId);
    const currChannelId = Number(currentChannelId);
    
    // Проверяем, что оба значения - валидные числа
    if (isNaN(msgChannelId) || isNaN(currChannelId)) {
      return false;
    }
    
    return msgChannelId === currChannelId;
  });
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      dispatch(addMessageFromSocket(sentMessage));
  ...
  // Если канал не найден, показываем заглушку
    ...
    return (
      <Container fluid className="p-0 d-flex flex-column" style={{ height: '100vh', overflow: 'hidden' }}>
        <Row className="g-0" style={{ flex: 1, minHeight: 0 }}>
          ...
          <Col md={9} lg={10} className="d-flex flex-column align-items-center justify-content-center">
            <h4>Канал не найден</h4>
            <p>Выберите другой канал из списка</p>
          </Col>
        </Row>
      </Container>
    );
  ...
  return (
    <Container fluid className="p-0 d-flex flex-column" style={{ height: '100vh', overflow: 'hidden' }}>
      <Row className="g-0" style={{ flex: 1, minHeight: 0 }}>
        ...
        <Col md={9} lg={10} className="d-flex flex-column" style={{ height: '100vh' }}>
          <div className="p-3 border-bottom flex-shrink-0">
            <h4># {currentChannel?.name}</h4>
          </div>

          {connectionStatus !== 'connected' && (
            <Alert variant="warning" className="m-3 mb-0 flex-shrink-0">
            ...
            <Form onSubmit={handleSendMessage}>
              <InputGroup>
                ...
                <Button
                  type="submit"
                  variant="primary"
                  disabled={!currentChannelId || !newMessage.trim() || sending}
                >
